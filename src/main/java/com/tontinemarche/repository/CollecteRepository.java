...
public interface CollecteRepository extends JpaRepository<Collecte, Long> {
    ...
    @Query("SELECT COALESCE(SUM(c.montantRecu), 0) FROM Collecte c WHERE c.dateCollecte = :date")
    BigDecimal sumByDate(@Param("date") LocalDate date);

    @Query("SELECT COALESCE(SUM(c.montantRecu), 0) FROM Collecte c WHERE c.agence.id = :agenceId AND c.dateCollecte = :date")
    BigDecimal sumByAgenceAndDate(@Param("agenceId") Long agenceId, @Param("date") LocalDate date);

    @Query("SELECT COALESCE(SUM(c.montantRecu), 0) FROM Collecte c WHERE c.agent.id = :agentId AND c.dateCollecte = :date")
    BigDecimal sumByAgentAndDate(@Param("agentId") Long agentId, @Param("date") LocalDate date);

    @Query("SELECT COALESCE(SUM(c.montantRecu), 0) FROM Collecte c WHERE c.agent.id = :agentId AND c.dateCollecte BETWEEN :debut AND :fin")
    ...
    @Query("SELECT COALESCE(SUM(c.montantRecu), 0) FROM Collecte c WHERE c.agence.id = :agenceId AND c.agent.id = :agentId AND c.dateCollecte = :date")
    BigDecimal sumByAgenceAndAgentAndDate(@Param("agenceId") Long agenceId, @Param("agentId") Long agentId, @Param("date") LocalDate date);

    @Query("SELECT COALESCE(SUM(c.montantRecu), 0) FROM Collecte c WHERE c.dateCollecte BETWEEN :debut AND :fin")
    BigDecimal sumBetween(@Param("debut") LocalDate debut, @Param("fin") LocalDate fin);

    @Query("SELECT COALESCE(SUM(c.montantRecu), 0) FROM Collecte c WHERE c.agence.id = :agenceId AND c.dateCollecte BETWEEN :debut AND :fin")
    BigDecimal sumByAgenceBetween(@Param("agenceId") Long agenceId, @Param("debut") LocalDate debut, @Param("fin") LocalDate fin);

    @Query("SELECT COALESCE(SUM(c.montantRecu), 0) FROM Collecte c WHERE c.client.id = :clientId")
